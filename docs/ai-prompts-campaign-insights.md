# AI Prompts — Campaign Insights & Optimization Recommendations

## Overview

Two AI-powered features on the campaign detail page:
1. **AI Diagnosis Insight** — The "Near Target" banner with a narrative summary of campaign health
2. **Optimization Recommendations** — Actionable suggestions (budget shifts, creative refreshes, targeting changes)

Both are generated by passing structured campaign data to the LLM with specific prompts.

---

## 1. AI Diagnosis Insight
...
Same payload as the diagnosis prompt, plus creative-level data:

```json
{
  "...same campaign data as above...",
...
| Trigger | Regenerate |
|---------|-----------|
| Campaign data refreshes (daily sync from ad platform) | Both diagnosis + recommendations |
| User applies a recommendation | Recommendations only (remove applied, may generate new ones) |
| User dismisses a recommendation | Neither (just hide it) |
| Campaign status changes (active → paused) | Diagnosis only |
| New ad set or creative added | Both |
| Manually triggered by user ("Refresh analysis") | Both |
...
- Show the last successfully generated diagnosis/recommendations
- Display a subtle "Last updated X hours ago" timestamp
- Never show a blank diagnosis — fall back to a rule-based calculation using the health score formula from the campaign health logic doc
...
- Recommendations: empty array (show "Recommendations will appear once we have enough performance data.")
